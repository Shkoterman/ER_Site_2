...
import { sendAirtableReg } from '../api/airtable/sendAirtableReg';
...
export const EventPage = (data) => {
  ...
  const sendQuickRegData = async () => {
    console.log('отправка реги(page)', tgNick, name, event.id);
    try {
      await sendAirtableReg({ tgNick, name, eventId: event.id });
      setShowQuickReg(false);
      setIsSubmitted(true);
    } catch (error) {
            ...
            {event.placeName && (
              <div className='bg-[#151516] rounded-2xl px-4 py-6 mt-4'>
                ...
                <div className='text-[#898989] mx-0 mt-2 mb-0 px-4 py-0 text-base font-[300] flex flex-col gap-0'>
                  {event.placeName}
                  {event.placeAdres && `, ${event.placeAdres}`} <br />{' '}
                  {event.dataTime}
                  {event.placeLink && (
