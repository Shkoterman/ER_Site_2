...
const CalendarColumns = () => {
  ...
  const handleFilterTagClick = (filter) => {
    if (filter === 'Все') {
      ...
      setFiltersTagSet((prevFilters) => {
        ...
        const allSame =
          Object.values(newFiltersCheck).every((val) => val === true) ||
          Object.values(newFiltersCheck).every((val) => val === false);
  ...
  const applyFilters = (filtersTagSet) => {
    const filtered = events.filter((event) => {
      // Фильтрация по тегам
      const isTagMatch = Object.entries(filtersTagSet).some(
        ([filterKey, isActive]) => {
        ...
        <div className='flex'>
          {Object.keys(groupedEvents).map((date) => {
            ...
            return (
              <div
                key={date}
                className='flex-shrink-0 snap-center px-1 border-r border-[#323232]/30 text-[#565656] text-center'
              >
                {/* День недели (жирный) */}
