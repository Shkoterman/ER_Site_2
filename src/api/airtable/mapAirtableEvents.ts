...
import { AirtableEventsData } from './types';

export const mapAirtableEvents = (data: EventsDTO): AirtableEventsData => {
  ...
  const tagsSetByEvents = new Set<string>();
  const timeSetByEvents = new Set<string>();
  ...
  return {
    ...
    events: data.map((record) => {
      ...
      if (!startDate) {
        // TODO
        throw new Error('Unexpected missing start date');
      }
      ...
      if (!dontShowTime && barcelonaStopData) {
