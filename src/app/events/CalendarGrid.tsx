...
import { AirtableEventsData } from '@/api/airtable/types';
...
export const CalendarGrid = ({
  ...
  const handleFilterTimeClick = (filter: string) => {
    const newFilters = Object.fromEntries(
      Object.keys(filtersTimeSet).map((key) => [key, false])
    );
    newFilters[filter] = true;
    setFiltersTimeSet(newFilters);
    updateURL(
      filter,
      Object.keys(filtersTagSet).find((key) => filtersTagSet[key]) || 'Все'
    );
  };
  ...
  const handleFilterTagClick = (filter: string) => {
    const newFilters = Object.fromEntries(
      Object.keys(filtersTagSet).map((key) => [key, false])
    );
    newFilters[filter] = true;
    setFiltersTagSet(newFilters);
    updateURL(
      Object.keys(filtersTimeSet).find((key) => filtersTimeSet[key]) ||
        'Всегда',
      filter
    );
  };
  ...
  const applyFilters = (
    ...
    const filtered = events.filter((event) => {
      const isTimeMatch =
        ...
        event.eventTimeList.some((time) => filtersTimeSet[time]);
      const isTagMatch =
        filtersTagSet['Все'] ||
        event.eventTagList.some((tag) => filtersTagSet[tag]);
  ...
  return (
    <div className='lg:flex flex-col gap-8 p-4 pb-24 bg-[#222221] text-[#70706c]'>
      <div className='py-12'>
        ...
        <div className='pt-2 text-2xl font-[300] text-[#676767]'>
