...
<div className="p-0 rounded rounded-md max-w-[560px] bg-[#171717]/70 hover:bg-[#000000] flex">
    ...
    <div class="h-[200px] w-1/3 bg-cover bg-center rounded-md flex flex-col items-left justify-end opacity-60 hover:opacity-100 cursor-pointer" style={{ backgroundImage: `url(${imageUrl})` }}>
    <div class="text-right p-2">
        <span class="inline-block bg-[#111111]/90 px-3 py-1 rounded-full text-xs">весь месяц</span>
    </div>
    </div>
    
    {/* Текстовая часть карточки */}
    
    <div class="h-48 w-2/3 flex flex-col justify-between px-4">
        <h3 class="text-white/80 text-[20px] font-[600] text-left leading-[22px] pt-2.5 pb-2">{title}</h3>
        {description && (
          <div class="flex-1 overflow-hidden text-left text-[13px] leading-[17px] text-[#676767] font-[300] tracking-wide">
            <p>{description}</p>
          </div>
        )}
      ...
      <div
        class="h-[220px] bg-cover bg-center rounded-md flex flex-col items-left justify-end hidden"
      ...
      <div class="px-3 py-2 text-sm backdrop-blur-sm hover:bg-black/50 rounded-md flex justify-between h-16 hover:h-full hover:pt-4 overflow-hidden">
            ...
            <div class="">
				 <h3 class="text-white/90 text-[17px] font-bold text-left leading-5">{title}</h3>
                 <div class="text-white text-left pt-2">
                    {time && <p className="hidden">{time}</p>}
                 </div>
                    {/* Описание с обрезкой, если оно больше 4 строк */}
        {description && (
          <div class="h-32 overflow-scroll text-left text-xs text-white/60 font-think">
            <p>{description}</p>
          </div>
        )}
            </div>
